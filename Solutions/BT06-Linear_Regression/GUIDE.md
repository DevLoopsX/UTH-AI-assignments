...
Tài liệu này giải thích chi tiết `advertise_LR_updated.ipynb` theo đúng thứ tự yêu cầu của đề bài.
...
= \frac{\partial}{\partial w_j}\left[\frac{1}{2}\big(e^{(i)}\big)^2\right]
...
$$
w_j\leftarrow w_j - lr\cdot e^{(i)}\cdot x_j^{(i)},\qquad j=0,1,2,3.
$$
...
$$
J_{\mathrm{MSE}}(\vec{w})=\frac{1}{2m}\sum_{i=1}^m\big(e^{(i)}\big)^2
=\frac{1}{2m}\sum_{i=1}^m\big(\hat{y}^{(i)}-y^{(i)}\big)^2.
$$

Đạo hàm riêng của $J_{\mathrm{MSE}}$ theo từng tham số $w_j$ với $j=0,1,2,3$:

$$
\frac{\partial J_{\mathrm{MSE}}}{\partial w_j}
= \frac{1}{m}\sum_{i=1}^m \frac{\partial}{\partial w_j}\left[\frac{1}{2}\big(e^{(i)}\big)^2\right]
= \frac{1}{m}\sum_{i=1}^m e^{(i)}\cdot x_j^{(i)}.
$$

Batch GD cập nhật theo công thức cho từng tham số:

$$
w_j\leftarrow w_j - lr\cdot\frac{1}{m}\sum_{i=1}^m e^{(i)}\cdot x_j^{(i)},\qquad j=0,1,2,3.
$$
...
Phần khởi tạo tham số giống Câu 1 để đảm bảo cùng điều kiện đầu vào cho hai phương pháp tối ưu, giúp so sánh trực quan hơn ở Câu 3. Ở mỗi epoch, Batch GD tính sai số vector `e = X @ w - y`, trong đó `X @ w` là $\hat{\mathbf y}$, rồi tính gradient bằng `grad = (X.T @ e) / m` tương ứng với công thức $\frac{1}{m}\sum_{i=1}^m e^{(i)}\cdot x_j^{(i)}$ cho mỗi tham số $w_j$. Sau đó cập nhật `w = w - lr * grad` đúng theo công thức Batch GD.
...
$$
\frac{\partial}{\partial w_j}\left|e^{(i)}\right|
= \mathrm{sign}\big(e^{(i)}\big)\cdot x_j^{(i)},\qquad j=0,1,2,3.
$$

Với toàn bộ dữ liệu, đạo hàm riêng của $J_{\mathrm{MAE}}$ theo tham số $w_j$ là:

$$
\frac{\partial J_{\mathrm{MAE}}}{\partial w_j}
= \frac{1}{m}\sum_{i=1}^m \mathrm{sign}\big(e^{(i)}\big)\cdot x_j^{(i)},\qquad j=0,1,2,3.
$$

### 4.2. SGD với MAE và giải thích

SGD với MAE dùng `np.sign(e_i)` để thay cho $e^{(i)}$ trong gradient của MSE.
...
### 4.3. Batch GD với MAE và giải thích

Batch GD với MAE tính `sign` trên toàn bộ vector sai số, sau đó tính tổng $\frac{1}{m}\sum_{i=1}^m \mathrm{sign}(e^{(i)})\cdot x_j^{(i)}$ cho mỗi tham số $w_j$.
